/* ============================================================
   RefineIQ — Chat interface (main screen)
   ============================================================ */

/* AI message card wrapper — supports card-style variants + streaming shimmer */
function AICard({ variant, streaming, children }) {
  const base = {
    position: "relative", borderRadius: "var(--r-lg)", padding: "20px 22px",
  };
  if (variant === "minimal") {
    return (
      <div className={"ai-sheen-host " + (streaming ? "ai-streaming" : "")} style={{
  ...
  if (variant === "bordered") {
    return (
      <div className={"ai-grad-border " + (streaming ? "flowing" : "") + " ai-sheen-host " + (streaming ? "ai-streaming" : "")}
...
function AIMessageRow({ msg, variant, streaming, onStreamDone, onOpenCite, animateCites }) {
  return (
      ...
      <div style={{ flex: 1, minWidth: 0 }}>
        <AICard variant={variant} streaming={streaming}>
          {streaming
            ...
            : <AnswerBody blocks={msg.blocks} />}
          {(!streaming) && msg.citations && (
            <Citations cites={msg.citations} onOpen={onOpenCite} animate={animateCites} />
          )}
          {(!streaming) && (
...
function ThinkingRow() {
  return (
    <div className="fade-up" style={{ display: "flex", gap: 14, maxWidth: 760, width: "100%" }}>
      <div style={{ flex: "none", paddingTop: 2 }}><Avatar ai size={34} /></div>
      <div className="glass" style={{ flex: 1, borderRadius: "var(--r-lg)", padding: "18px 22px",
...
function ChatScreen({ dept, cardVariant }) {
  const [thread, setThread] = React.useState(() => SEED_THREAD.map(m => ({ ...m })));
  const [phase, setPhase] = React.useState("idle"); // idle | thinking | streaming
  const [input, setInput] = React.useState("");
  const [focused, setFocused] = React.useState(false);
  const [pending, setPending] = React.useState(null); // the streaming msg
  const scrollRef = React.useRef(null);

  const scrollBottom = () => requestAnimationFrame(() => {
    const el = scrollRef.current; if (el) el.scrollTop = el.scrollHeight;
  });
  React.useEffect(() => { scrollBottom(); }, [thread.length, phase]);

  const ask = (q) => {
    if (!q.trim() || phase !== "idle") return;
    setInput("");
    setThread(t => [...t, { id: "u" + Date.now(), role: "user", text: q }]);
    setPhase("thinking");
    scrollBottom();
    setTimeout(() => {
      // build blocks from DEMO_ANSWER
      const a = DEMO_ANSWER;
      const blocks = [
        { type: "p", text: a.paragraphs[0] },
        ...a.steps.map(s => ({ type: "step", h: s.h, b: s.b })),
    ...
    <div style={{ display: "flex", flexDirection: "column", height: "100%", minWidth: 0 }}>
      ...
      <header style={{ flex: "none", height: 64, display: "flex", alignItems: "center", gap: 14,
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
          <span style={{ width: 30, height: 22, borderRadius: 6, display: "grid", placeItems: "center",
            fontSize: 10.5, fontWeight: 700, color: "#fff", background: "var(--ai-grad)" }}>{deptShort(dept)}</span>
          <div>
        ...
        <div style={{ flex: 1 }} />
        <div style={{ display: "flex", alignItems: "center", gap: 8, height: 36, padding: "0 13px",
          ...
          <span style={{ marginLeft: "auto", fontSize: 11, color: "var(--text-4)", border: "1px solid var(--border)",
            borderRadius: 5, padding: "1px 5px" }}>⌘K</span>
        </div>
      ...
      <div ref={scrollRef} style={{ flex: 1, overflowY: "auto", padding: "28px 26px 8px" }}>
        <div style={{ maxWidth: 820, margin: "0 auto", display: "flex", flexDirection: "column", gap: 26, paddingBottom: 10 }}>
          {thread.length <= 2 && (
            <div className="fade-in" style={{ display: "flex", alignItems: "center", gap: 10, alignSelf: "center",
          ...
          {thread.map(m => m.role === "user"
            ...
            : <AIMessageRow key={m.id} msg={m} variant={cardVariant} streaming={false}
                onOpenCite={() => {}} animateCites={m._justFinished} />
          )}
        ...
        <div style={{ maxWidth: 820, margin: "0 auto" }}>
          {phase === "idle" && (
            <div style={{ display: "flex", gap: 8, marginBottom: 12, flexWrap: "wrap" }}>
              {SUGGESTIONS.map((s, i) => (
                ...
                onMouseEnter={e => { e.currentTarget.style.borderColor = "var(--border-ai)"; e.currentTarget.style.color = "var(--text-1)"; }}
          ...
          <div style={{
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
              <span style={{ display: "inline-flex", alignItems: "center", gap: 5, height: 28, padding: "0 9px",
              ...
              <button onClick={() => ask(input)} disabled={!input.trim() || phase !== "idle"} className="focusable" style={{
              ...
              onMouseEnter={e => { if (input.trim() && phase === "idle") e.currentTarget.style.boxShadow = "0 8px 28px -4px rgba(139,92,246,0.95)"; }}
              onMouseLeave={e => { if (input.trim() && phase === "idle") e.currentTarget.style.boxShadow = "0 6px 20px -6px rgba(124,109,245,0.8)"; }}>
                <I.send size={18} />
              </button>
            </div>
          </div>
...
/* Streaming row — shows body streaming, then citations + actions fade in after */
function AIMessageRowStreaming({ msg, variant, onStreamDone }) {
  const [bodyDone, setBodyDone] = React.useState(false);
  return (
    <div className="fade-up" style={{ display: "flex", gap: 14, maxWidth: 760, width: "100%" }}>
      <div style={{ flex: "none", paddingTop: 2 }}><Avatar ai size={34} /></div>
      <div style={{ flex: 1, minWidth: 0 }}>
        <AICard variant={variant} streaming={!bodyDone}>
          <StreamingBody blocks={msg.blocks} onComplete={() => setBodyDone(true)} />
          {bodyDone && <Citations cites={msg.citations} animate onOpen={() => {}} />}
...
function FinishSignal({ onFinish }) {
  React.useEffect(() => { const t = setTimeout(onFinish, 600); return () => clearTimeout(t); }, []);
  return null;
}
