/* ============================================================
   RefineIQ — App sidebar (collapsible, context-aware)
   ============================================================ */

function NavItem({ icon: Ico, label, active, collapsed, onClick, badge }) {
  return (
    <button className="focusable" onClick={onClick} title={collapsed ? label : undefined} style={{
    ...
    onMouseLeave={e => { if (!active) e.currentTarget.style.background = "transparent"; }}>
      {active && <span style={{ position: "absolute", left: collapsed ? 6 : 0, top: "50%", transform: "translateY(-50%)",
...
function DeptSelector({ value, onChange, collapsed }) {
  const [open, setOpen] = React.useState(false);
  const cur = DEPARTMENTS.find(d => d.id === value) || DEPARTMENTS[0];
  if (collapsed) {
    return (
      <div title={cur.name} style={{ display: "grid", placeItems: "center", height: 40, margin: "0 auto", width: 40,
        borderRadius: 10, background: "var(--glass-faint)", border: "1px solid var(--border)",
        fontSize: 11, fontWeight: 700, color: "var(--text-2)" }}>{cur.short}</div>
    );
  }
  return (
    <div style={{ position: "relative" }}>
      <button className="focusable" onClick={() => setOpen(o => !o)} style={{
      ...
      onMouseLeave={e => e.currentTarget.style.borderColor = "var(--border)"}>
        <span style={{ width: 26, height: 18, borderRadius: 5, display: "grid", placeItems: "center",
          fontSize: 9.5, fontWeight: 700, color: "#fff", background: "var(--ai-grad)", flex: "none" }}>{cur.short}</span>
        <span style={{ flex: 1, textAlign: "left", fontSize: 13, fontWeight: 500 }}>{cur.name}</span>
        <I.chevD size={15} style={{ color: "var(--text-3)", transform: open ? "rotate(180deg)" : "none", transition: "transform var(--dur)" }} />
      </button>
      {open && (
        <div className="glass fade-in" style={{ position: "absolute", top: 46, left: 0, right: 0, zIndex: 40,
          ...
          {DEPARTMENTS.map(d => (
            ...
            onMouseEnter={e => { if (d.id !== value) e.currentTarget.style.background = "var(--bg-hover)"; }}
...
function Sidebar({ view, setView, collapsed, setCollapsed, dept, setDept, activeConv, setActiveConv, adminTab, setAdminTab, onLogout }) {
  const sectionLabel = (t) => !collapsed && (
    <div style={{ fontSize: 10.5, fontWeight: 700, letterSpacing: "0.08em", color: "var(--text-4)",
  ...
  return (
    <aside style={{
      ...
      {/* Primary nav */}
      <div style={{ padding: collapsed ? "8px 14px" : "8px 14px", display: "flex", flexDirection: "column", gap: 3 }}>
      ...
      {/* Context section */}
      <div style={{ flex: 1, overflowY: "auto", padding: collapsed ? "0 14px" : "0 14px", minHeight: 0 }}>
        {view === "chat" && (
          <>
            <button className="focusable" onClick={() => setActiveConv(null)} style={{
            ...
            {!collapsed && <div style={{ marginBottom: 16 }}>
              ...
              <DeptSelector value={dept} onChange={setDept} collapsed={collapsed} />
            </div>}
            {collapsed && <div style={{ marginBottom: 14 }}><DeptSelector value={dept} onChange={setDept} collapsed /></div>}
              ...
              <>
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: 2 }}>
                  {CONVERSATIONS.map(c => (
                    ...
                    onMouseEnter={e => { if (activeConv !== c.id) e.currentTarget.style.background = "var(--glass-faint)"; }}
                    onMouseLeave={e => { if (activeConv !== c.id) e.currentTarget.style.background = "transparent"; }}>
                      {c.pinned ? <I.pin size={13} style={{ flex: "none", color: "var(--violet)" }} /> : <I.chat size={13} style={{ flex: "none", opacity: 0.5 }} />}
                      <span style={{ flex: 1, fontSize: 13, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap", lineHeight: 1.3 }}>{c.title}</span>
        ...
        {view === "admin" && (
          <>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 3 }}>
              {ADMIN_NAV.map(n => (
                <NavItem key={n.id} icon={n.icon} label={n.label} active={adminTab === n.id}
                  collapsed={collapsed} onClick={() => setAdminTab(n.id)} />
              ))}
            </div>
          </>
        )}
            ...
            <>
              <div style={{ flex: 1, minWidth: 0 }}>
                <div style={{ display: "flex", alignItems: "center", gap: 7 }}>
                  <span style={{ fontSize: 13, fontWeight: 600, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{CURRENT_USER.name}</span>
                  <RoleBadge role={CURRENT_USER.role} />
                </div>
                <div style={{ fontSize: 11.5, color: "var(--text-3)", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{CURRENT_USER.email}</div>
              </div>
