/* ============================================================
   RefineIQ — Admin panel (users table + document library)
   ============================================================ */
...
function UsersTable() {
  const [users, setUsers] = React.useState(() => USERS.map(u => ({ ...u })));
  const [q, setQ] = React.useState("");
  const filtered = users.filter(u => (u.name + u.email + u.role).toLowerCase().includes(q.toLowerCase()));
  const toggle = (i) => setUsers(us => us.map((u, j) => j === i ? { ...u, status: !u.status } : u));

  return (
    <div className="glass" style={{ borderRadius: "var(--r-lg)", overflow: "hidden" }}>
      {/* Toolbar */}
      <div style={{ display: "flex", alignItems: "center", gap: 12, padding: "16px 20px", borderBottom: "1px solid var(--border)" }}>
        <div style={{ display: "flex", alignItems: "center", gap: 9, height: 38, padding: "0 13px", flex: 1, maxWidth: 320,
        ...
        <button style={{ display: "flex", alignItems: "center", gap: 7, height: 38, padding: "0 13px", borderRadius: 10,
          background: "var(--glass-faint)", border: "1px solid var(--border)", color: "var(--text-2)", fontSize: 13 }}>
          <I.filter size={15} /> Role
        </button>
        <div style={{ flex: 1 }} />
        <button className="focusable" style={{ display: "flex", alignItems: "center", gap: 8, height: 38, padding: "0 16px", borderRadius: 10,
      ...
      {/* Header */}
      <div style={{ display: "grid", gridTemplateColumns: "2.2fr 1.2fr 1.4fr 0.9fr 0.6fr", gap: 16, padding: "12px 22px",
      ...
      {/* Rows */}
      {filtered.map((u, i) => (
        <div key={u.email} style={{ display: "grid", gridTemplateColumns: "2.2fr 1.2fr 1.4fr 0.9fr 0.6fr", gap: 16,
          alignItems: "center", padding: "13px 22px", borderBottom: i < filtered.length - 1 ? "1px solid var(--border-soft)" : "none",
          transition: "background var(--dur) var(--ease)" }}
          onMouseEnter={e => e.currentTarget.style.background = "var(--glass-faint)"}
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 9 }}>
            <Toggle on={u.status} onChange={() => toggle(users.indexOf(u))} />
            <span style={{ fontSize: 12, color: u.status ? "var(--text-2)" : "var(--text-4)" }}>{u.status ? "Active" : "Suspended"}</span>
          </div>
...
function DocLibrary() {
  const [q, setQ] = React.useState("");
  const filtered = DOCUMENTS.filter(d => (d.name + d.dept).toLowerCase().includes(q.toLowerCase()));
  return (
    <div className="glass" style={{ borderRadius: "var(--r-lg)", overflow: "hidden" }}>
      <div style={{ display: "flex", alignItems: "center", gap: 12, padding: "16px 20px", borderBottom: "1px solid var(--border)" }}>
        <div style={{ display: "flex", alignItems: "center", gap: 9, height: 38, padding: "0 13px", flex: 1, maxWidth: 320,
      ...
      <div style={{ display: "grid", gridTemplateColumns: "2.4fr 1.4fr 0.7fr 1fr 1fr 0.5fr", gap: 16, padding: "12px 22px",
        fontSize: 11, fontWeight: 700, letterSpacing: "0.05em", textTransform: "uppercase", color: "var(--text-4)",
        borderBottom: "1px solid var(--border-soft)" }}>
        <span>Document</span><span>Department</span><span>Version</span><span>Updated</span><span>Status</span><span></span>
      </div>
      {filtered.map((d, i) => (
        <div key={d.name} style={{ display: "grid", gridTemplateColumns: "2.4fr 1.4fr 0.7fr 1fr 1fr 0.5fr", gap: 16,
          alignItems: "center", padding: "13px 22px", borderBottom: i < filtered.length - 1 ? "1px solid var(--border-soft)" : "none" }}
          onMouseEnter={e => e.currentTarget.style.background = "var(--glass-faint)"}
          onMouseLeave={e => e.currentTarget.style.background = "transparent"}>
          ...
          <div style={{ display: "flex", justifyContent: "flex-end" }}><IconBtn icon={I.more} size={32} title="More" /></div>
        </div>
      ))}
    </div>
  );
...
function Placeholder({ tab }) {
  ...
  return (
    <div className="glass" style={{ borderRadius: "var(--r-lg)", padding: 60, display: "flex", flexDirection: "column",
      alignItems: "center", justifyContent: "center", textAlign: "center", minHeight: 360 }}>
      <div style={{ width: 56, height: 56, borderRadius: 16, display: "grid", placeItems: "center", marginBottom: 18,
        background: "var(--indigo-soft)", border: "1px solid var(--border-ai)", color: "#B9A6FA" }}>
        <p.icon size={26} />
      </div>
